...
int main(int argc, char** argv) {
    ...
    if (cmd_option_exists(argv, argv + argc, "-h") || argc == 1) {
        std::cout << "Usage: ./kmeans_cuda -input <file.csv> -k <num_clusters> [-max_iter <iters>]\n";
        return 0;
    }
    ...
    try {
        // 2. Load the Dataset
        // Reused data structure from GPU/OpenMP version.
        ...
        print_execution_summary("CUDA", data.n, iterations, elapsed.count());
